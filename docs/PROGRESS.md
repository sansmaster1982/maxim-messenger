...
### Доработка v0.1.4 — надёжный маршрут + подтверждение вживую
v0.1.3 на устройстве всё ещё слал по chatId: hint-механизм (`dialogPeerHintProvider`) оказался хрупким по таймингу (чат открыли из списка, build контроллера закеширован → ensureExists с peerUserId не отрабатывал). Фикс: в `_resolveRoute` добавлен фолбэк — если у строки нет ни `server_chat_id`, ни `peer_user_id`, но её id есть в локальных контактах → это диалог 1:1, слать по `userId`. Группы не задеты (у них `server_chat_id` из синка списка, ветка выше).

Живой тест (телефон, новая SIM): `REQ op=64 userId:<userId> → RESP cmd=1 {chatId: <chatId>, message:{id, cid, text:тест}}`. Сообщение принято сервером. Подтвердилось, что **<chatId> — это серверный id диалога** (тот самый «User <chatId>» из прежних ошибок `user.not.found`): клиент слал туда user-id контакта, сервер не находил. Реконсиляция берёт `res['chatId']` → дальше шлём по chatId. Известный шов: `chatHistory`/`loadOlder` пока ходят по локальному id (история диалога с сервера не подтягивается до доработки роутинга чтения).

---

## Этап 11 — Recovery после обрыва + приём + имя чата (v0.1.5) (2026-06-06)

Живой тест: и отправка, и приём в диалоге 1:1 работают (op 64 → cmd=1; входящее `PUSH op 128` отображается — подтверждён op 128 = NEW_MESSAGE). Всплыли три дефекта при нестабильной сети.

### 1. Reconnect глох навсегда
`_ReconnectManager._cancelled` ставился в true при мёртвом токене (MaxLoginFailed) и НЕ сбрасывался. После `FAIL_LOGOUT_ALL` + повторного входа любой обрыв сети оставлял сокет null — сообщения висли в очереди, клиент «не подключался». Фикс: `rearm()` снимает `_cancelled` при каждом успешном LOGIN.

### 2. «Висит в переподключении»
`authThrottle` 90с между LOGIN на флаппинг-сети выглядел как зависание (приложение молча ждёт таймер, лог пуст). Снижено до 30с (предохранитель флаппинга по-прежнему ловит шторм). `syncHistory`/`loadOlder` больше не бросают Unhandled при offline — отдают локальную историю.

### 3. Имя чата — число
Строка диалога имела плейсхолдер «Чат N». `listLocal` подставляет имя контакта для диалогов с плейсхолдерным/пустым title.

Корень нестабильности соединения — сеть/VPN на устройстве (errno 103 + сбои DNS), не клиент. Приложение теперь переживает обрывы и досылает очередь; для плавной работы нужна стабильная сеть.

### Проверка
`flutter analyze` чисто, 34 теста. Вживую: после re-arm приложение переподключается, сообщение уходит (`op 64 chatId:<chatId>`). Релиз `v0.1.5`.
